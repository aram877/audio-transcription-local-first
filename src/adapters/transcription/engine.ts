...
import type { ProgressCallback, Transcript, TranscriptSegment } from '../../core/types';
...
function isCorruptModelError(message: unknown): boolean {
...
// The transformers.js ASR pipeline object (kept loosely typed; the library has
// no stable public TS surface for the pipeline internals we touch).
...
export async function getPipeline(
  ...
  _pipelinePromise = (async () => {
    ...
    try {
      ...
      if (isCorruptModelError(underlying)) {
...
export async function transcribe(pcm: Float32Array, opts: EngineTranscribeOptions = {}): Promise<Transcript> {
  if (!pcm || pcm.length === 0) {
  ...
  try {
    ...
    throw new Error(`Transcription failed: ${err && err.message ? err.message : err}`);
  }

  const chunks: any[] = Array.isArray(output && output.chunks) ? output.chunks : [];
  const segments: TranscriptSegment[] = chunks.map((c) => ({
 ...
 */
export async function detectLanguage(asr: AsrPipeline, pcm: Float32Array): Promise<string | null> {
  try {
